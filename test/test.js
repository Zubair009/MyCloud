const MyCloud = artifacts.require('./MyCloud.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('MyCloud', ([deployer, uploader]) => {
  let mycloud

  before(async () => {
    mycloud = await MyCloud.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await mycloud.address
    ...
    it('has a name', async () => {
      const name = await mycloud.name()
      assert.equal(name, 'MyCloud')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await mycloud.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await mycloud.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await mycloud.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await mycloud.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await mycloud.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await mycloud.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await mycloud.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await mycloud.files(fileCount)
